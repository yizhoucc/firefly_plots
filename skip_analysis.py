...
with initiate_plot(6,3,300) as f:
    ...
    plt.scatter([t[1]*400 for t in tasks],[t[0]*400 for t in tasks], label='all', s=0.5)
    ax.axis('equal')
    ax.title('all')
  
    ax=f.add_subplot(122)
    plt.scatter(df[df.category=='skip'].target_x,df[df.category=='skip'].target_y, label='skip',s=1)
    ax.axis('equal')
    ax.title('skips')
    

# get all posible depent vars -----------------------------
# value of all tasks   
...
while r<len(encodedtrialtype):
    if encodedtrialtype[l]==pre:
        r=l
        while r<len(encodedtrialtype) and encodedtrialtype[r]==pre:
            r+=1
            l+=1
...
# plot, skip value vs nonskip values
...
# plot, distance and angles instead of value
# distant
skipds=[]
for i in range(len(encodedtrialtype)):
    if encodedtrialtype[i]==trialtypeencoding['skip']:
...
# chage the lrdata generating part to use different dependent variables
accs=[]
accsstd=[]
